...
public class ASCLBasicController : MonoBehaviour 
{
	...
	void LateUpdate () 
	{
        ...
        Collider[] colliders = Physics.OverlapSphere(transform.position, radius + 2.5f, 1 << LayerMask.NameToLayer("EndPoint"));
        ...
        if (colliders.Length > 0 || totalTime <= 0)
        {
            winText.text = "Your Score:" + count.ToString();
            endGame.gameObject.SetActive(true);
            string Name = "Admin";
            scoreList.Add(new Score(Name, count));
        ...
        Vector3 newPos = transform.position + (transform.rotation * Vector3.forward * moveSpeed);
		...
		if (Input.GetMouseButtonUp(0))//ok, we can clear the right mouse button and use it for the next attack
		{
	...
	void OnGUI()
	{
		string tempString = "Your Score:" + count.ToString();
		GUI.Label (new Rect (10, 5,1000, 20), tempString);
        string timeText = "Your Time:" + totalTime.ToString();
        GUI.Label(new Rect(10, 25, 1000, 20), timeText);
    }
	...
	void CheckForHit(Enemy en, AbilityCollision ac)
	{
		...
		if(ac.type==0)
		{
            ...
            Vector3 distance = transform.position - en.gameObject.transform.position;
			...
			if (tAngle<angle)
			{
				if(Vector3.Distance(transform.position, en.gameObject.transform.position)<ac.range)
				{
                    //we have a hit
                    //AngleRanged
                    en.gameObject.transform.position -= distance;
